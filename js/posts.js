...
const POSTS = [
  {
    ...
    tags: ["비밀의정원", "운영", "결정"],
    content: `
여러 사업을 동시에 굴리다 보면 가장 먼저 흐려지는 게 **오프라인 매장의 기본 규칙**이다. 매일 돌아가는 곳이라 "알아서 되겠지"가 되기 쉽다. 오늘 비밀의정원의 운영 원칙을 다시 세우고 숫자로 못 박았다. **시행은 10월 1일부터.** 9월은 준비 기간이다.

## 결정한 것

**1. 인건비 상한 — 월 600만원 이하 (10월부터)**
가게 인건비(급여 총액)를 월 600만원 이하로 맞춘다. 바앱(바텐톡) 관련 비용도 이 안에 포함해서 관리한다. 매장 하나가 감당할 수 있는 고정비의 선을 먼저 그은 것.

**2. 급여를 순이익의 10%로 전환 (10월부터)**
고정 급여 구조에서 성과 연동 구조로 바꾼다. 가게가 잘 되면 같이 잘 되고, 어려우면 같이 버티는 구조. 9월 한 달 동안 순이익 계산 기준을 정하고, 10월 1일부터 적용한다.

**3. 영업 루틴 — 화~일, 18:30 ~ 02:30 (10월부터)**
주 6일 영업, 월요일 휴무 + 격주 일요일 휴무. 새벽 2시 라스트오더, 2시부터 마감 시작. "언제 열고 언제 닫는지"를 고정해야 직원도 손님도 나도 예측이 된다.

**4. 사장 출근 시간 고정 — 밤 9시~11시 (일요일은 8시~1시) (10월부터)**
...
- **10월 1일**: 네 가지 원칙 동시 시행 — 인건비 상한, 급여 전환, 영업 루틴, 출근 시간
- **11월 1일**: 시행 한 달 점검 — 인건비 600만 이하가 지켜졌는지, 루틴이 유지됐는지, 급여 전환에 문제가 없었는지 돌아보기
